 ...
 *
 * <p>
 * Nothing here writes to the library. The dry run's every side effect is blocked
 * inside the executor, which is why this job needs no ownership of the folders
 * it reads.
 */
@Slf4j
@Component
class OrganizationPreviewJob {
	...
	void run(OrganizationExecuteRequest request, Execution execution) {
		Long executionId = execution.getId();

		AtomicReference<OrganizationPlan> planned = new AtomicReference<>();

		try {
			organizationExecutor.execute(request, execution, null, plan -> {
				planned.set(plan);
