...
public class MetadataRebuildJobHandler implements ExecutionJobHandler {
	...
	public void handle(Execution execution, ClaimedExecution claimed, ExecutionOwnership ownership) {
		...
		if (payload.dryRunValue()) {
			simulate(execution, ownership, payload);

			return;
		}

		rebuild(execution, ownership, payload);
	}
	 ...
	 */
	private void simulate(Execution execution, ExecutionOwnership ownership, MetadataRebuildPayload payload) {
	 ...
	 */
	private void rebuild(Execution execution, ExecutionOwnership ownership, MetadataRebuildPayload payload) {
		MetadataRebuildRequest request = payload.toRequest();

		executionProgressService.updateTotal(ownership, (int) metadataRebuildService.countCandidates(request));
		...
		executionProgressService.finishCommand(ownership, outcome(execution, result),
