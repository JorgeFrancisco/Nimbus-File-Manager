...
public final class FileNameDatePatterns {
	...
	public static final Pattern DATE8_UNDERSCORE_TIME6 = Pattern.compile(".*?(\\d{8})_(\\d{6}).*", Pattern.CASE_INSENSITIVE);

	/** {@code yyyyMMdd} then {@code _}/{@code -} then {@code HHmmss}. */
	public static final Pattern DATE8_SEP_TIME6 = Pattern.compile(".*?(\\d{8})[_-](\\d{6}).*", Pattern.CASE_INSENSITIVE);

	/** Bare {@code yyyyMMdd} block anywhere in the name. */
	public static final Pattern DATE8 = Pattern.compile(".*?(\\d{8}).*", Pattern.CASE_INSENSITIVE);
