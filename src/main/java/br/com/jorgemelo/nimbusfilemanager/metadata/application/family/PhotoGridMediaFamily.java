...
public class PhotoGridMediaFamily extends AbstractFileNameDateRule implements MediaSubcategoryRule {
	...
	private static final Pattern EPOCH_MILLIS = Pattern.compile("PHOTOGRID[-_](\\d{13})", Pattern.CASE_INSENSITIVE);
	...
	public LocalDateTime resolve(String fileName) {
		LocalDateTime epoch = fromEpochMillis(fileName);
	...
	private LocalDateTime fromEpochMillis(String fileName) {
		if (fileName == null) {
			return null;
		}

		Matcher matcher = EPOCH_MILLIS.matcher(fileName);
