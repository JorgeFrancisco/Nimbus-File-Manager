 ...
 * (which drives the progress bar and the post-encode validation) and the capture
 * date already resolved for it (which the converted file inherits when its own
 * re-extraction can only offer a filesystem timestamp). Read separately from the
 * entity because all of it lives on lazily fetched associations.
 */
