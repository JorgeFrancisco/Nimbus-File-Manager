...
public class InventoryBatchWriter {
	...
	public void write(Execution execution, InventoryScanRequest request, List<Path> files,
		...
		for (InventoryBatchItemResult result : results) {
			...
			if (result.result().action() == InventoryPersistenceAction.REACTIVATED) {
				counters.countReactivated();
			}
		}

		executionProgressService.updateProgress(ownership, counters.found(), counters.analyzed(), counters.cacheHits(),
