 ...
 * native {@link WindowsChangeSourceSupport#open} entry point, so the
 * application layer's provider never imports this infrastructure glue directly.
 * Lives with the native code (excluded from coverage) and only touches native
 * code on Windows, when the provider actually calls {@code open}.
 */
@Component
