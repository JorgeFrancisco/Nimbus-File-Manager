 ...
 * @param mode how the answer is to be reached. Absent means {@code REBUILD},
 * which is what every request written before the incremental path existed meant
 * - so an execution queued by an older version still runs, and runs as what it
 * asked for
 */
@JsonIgnoreProperties(ignoreUnknown = true)
public record SimilarityAnalysisPayload(Integer schemaVersion, Integer minSimilarityPercent,
		String expectedParametersDigest, String expectedCompositionDigest, SimilarityRunMode mode) {
}