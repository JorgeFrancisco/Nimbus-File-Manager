...
public class PhotoSimilarityJobHandler implements ExecutionJobHandler {
	...
	/**
	 * The ownership goes unused: this reads fingerprints and writes rows of its
	 * own. No file of the user's is touched, so there is no irreversible write to
	 * guard between batches.
	 */
	@Override
	public void handle(Execution execution, ClaimedExecution claimed, ExecutionOwnership ownership) {
		similarityJob.run(photoSimilarityService, execution, claimed);
	}
}