-- The replay branch of a folder relocation stops throwing.
--
-- When provenance became one object, the parameters it replaced went away - and
-- one reference to the old p_source survived, in the branch that recognises a
-- retry re-applying facts it had already written. That branch never runs on a
-- first attempt, which is why the whole of this work applied folder relocations
-- correctly and nothing noticed: the only way in is a worker that died between
-- writing the facts and recording that it had, and then came back.
--
-- What it did on that path was raise "column p_source does not exist", turning
-- the one situation the identities exist to make safe into an error. The
-- function is recreated here with the reference the composite type replaced it
-- with; nothing else about it changes.

CREATE OR REPLACE FUNCTION public.relocate_catalog_folder_contents(p_catalog_file_ids bigint[], p_catalog_file_event_public_ids uuid[], p_old_root text, p_new_root text, p_path_flavor text, p_provenance catalog_fact_provenance)
 RETURNS TABLE(files_relocated integer, replayed boolean)
 LANGUAGE plpgsql
AS $function$
DECLARE
    v_old_key TEXT;
...
BEGIN
    ...
    IF v_known = v_supplied THEN
        IF EXISTS (
            ...
            WHERE e.catalog_file_id <> i.catalog_file_id OR e.source IS DISTINCT FROM (p_provenance).source
        ) THEN
