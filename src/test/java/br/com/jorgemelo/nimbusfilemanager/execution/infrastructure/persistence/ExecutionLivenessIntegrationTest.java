...
class ExecutionLivenessIntegrationTest {
	...
	void renewingKeepsItAliveAndOnlyItsOwnerMayRenew() {
		...
		Assertions.assertThat(executionQueue.renewLeases("worker-b", List.of(queued.getId()), LEASE_SECONDS))
				.isZero();
		Assertions.assertThat(executionQueue.renewLeases("worker-a", List.of(queued.getId()), LEASE_SECONDS))
				.isEqualTo(1);
		Assertions.assertThat(executionQueue.expiredLeases()).doesNotContain(queued.getId());
	}
