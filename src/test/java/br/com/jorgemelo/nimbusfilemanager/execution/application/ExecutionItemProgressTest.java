...
class ExecutionItemProgressTest {
	...
	void recordsHowFarIntoTheCurrentItemTheWorkHasGot() {
		...
		service().updateCurrentItem(execution, 42);
	...
	void holdsBackAChangeThatArrivesWithinTheWindow() {
		...
		service.updateCurrentItem(execution, 10);
		service.updateCurrentItem(execution, 11);
	...
	void keepsThePercentageWithinItsOwnRange() {
		...
		service().updateCurrentItem(execution, 140);
	...
	void clearsWhatWasMidItemWhenTheExecutionEnds() {
		...
		service.updateCurrentItem(execution, 42);
		service.fail(execution, ExecutionMessages.executionInterrupted());
