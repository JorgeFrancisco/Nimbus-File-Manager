...
class InventoryScanRunnerTest {
	 ...
	 * through means the catalog reflects everything that existed before it
	 * started, so the position read before it started is now a safe place for the
	 * next start to replay from.
	 */
	@Test
	 ...
	 * so the older cursor stands and the next start replays a wider window. That
	 * is the trade this whole mechanism is built on: repeat rather than miss.
	 */
	@Test
	 ...
	 * still using - and what gets stored has to be where it was <em>before</em>
	 * the walk. Storing where it ended would claim the walk had seen changes that
	...
	void aPassThatBroughtFilesBackAsksForOneRegroup() {
		...
		doAnswer(invocation -> {
			...
			return null;
		}).when(inventoryBatchWriter).write(any(), any(), any(), any(), any());
	...
	void writesNothingWhenTheLibraryIsEmpty() {
		...
		verify(inventoryBatchWriter, never()).write(any(), any(), any(), any(), any());
