...
class FileNotifyExtendedInformationParserTest {

	@Test
	void parsesASingleRelativePath() {
		Assertions.assertThat(FileNotifyExtendedInformationParser.parse(FileNotifyBuffers.buffer("2024\\05\\photo.jpg"))).extracting(FileNotifyEntry::relativePath)
.containsExactly("2024\\05\\photo.jpg");
	...
	void parsesEveryChainedEntryInOrder() {
		...
		Assertions.assertThat(FileNotifyExtendedInformationParser.parse(buffer)).extracting(FileNotifyEntry::relativePath)
.containsExactly("a.jpg", "sub\\b.png",
				"sub\\deep\\c.mp4");
	}

	@Test
	void decodesUtf16Names() {
		Assertions.assertThat(FileNotifyExtendedInformationParser.parse(FileNotifyBuffers.buffer("férias\\praia.jpg"))).extracting(FileNotifyEntry::relativePath)
.containsExactly("férias\\praia.jpg");
	}

	@Test
	void returnsEmptyForNullOrShortBuffers() {
		Assertions.assertThat(FileNotifyExtendedInformationParser.parse(null)).isEmpty();
		Assertions.assertThat(FileNotifyExtendedInformationParser.parse(new byte[4])).isEmpty();
	}
	...
	void reportsAPathForEveryActionAndNotOnlyForAddedOnes() {
		...
		Assertions.assertThat(FileNotifyExtendedInformationParser.parse(buffer)).extracting(FileNotifyEntry::relativePath)
.containsExactly("added.jpg", "removed.jpg",
