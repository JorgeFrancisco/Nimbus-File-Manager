...
class SimilarityCompositionAgreementTest {
	...
	private PhotoHashRawResponse photo(UUID id, String folder) {
		return new PhotoHashRawResponse(id, new byte[32], new byte[64], "photo.jpg", "jpg", 100L,
				folder + "/photo.jpg", folder, NOW);
	}

	private VideoFrameRawResponse frame(UUID id, int sampleIndex) {
		return new VideoFrameRawResponse(id, sampleIndex, (long) sampleIndex * 1000, new byte[32], new byte[64],
				"video.mp4", "mp4", 100L, "C:/Videos/video.mp4", "C:/Videos", NOW, 10.0, 1920, 1080);
	}
}