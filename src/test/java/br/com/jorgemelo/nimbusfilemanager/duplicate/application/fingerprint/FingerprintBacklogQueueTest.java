...
class FingerprintBacklogQueueTest {
	...
	void aDrainIsSafeToRunAgainFromTheStart() {
		...
		photos.handle(execution(), claimed(payload(1, false)), null);
		photos.handle(execution(), claimed(payload(1, false)), null);

		verify(photoBacklog, times(2)).drainPending(any(), any());
		verify(photoBacklog, never()).rebuild();
	}

	@Test
	void aRebuildDiscardsWhatExistsBeforeDraining() {
		whenDraining(photoBacklog, new DrainResult(10, 0));

		photos.handle(execution(), claimed(payload(1, true)), null);

		verify(photoBacklog).rebuild();
		verify(photoBacklog).drainPending(any(), any());
	}
	...
	void aDrainClaimedWhileAnInventoryIsRunningStepsAside() {
		...
		verify(photoBacklog, never()).drainPending(any(), any());
		verify(photoBacklog, never()).rebuild();
	...
	void aCleanDrainFinishes() {
		...
		videos.handle(execution(), claimed(payload(1, false)), null);
	...
	void progressIsReportedAgainstTheRow() {
		when(photoBacklog.status()).thenReturn(new FingerprintBacklogStatus(120, 0, 0));
		when(photoBacklog.drainPending(any(), any())).thenAnswer(call -> {
	...
	private void whenDraining(FingerprintBacklog backlog, DrainResult result) {
		...
		when(backlog.drainPending(any(), any())).thenReturn(result);
	}
