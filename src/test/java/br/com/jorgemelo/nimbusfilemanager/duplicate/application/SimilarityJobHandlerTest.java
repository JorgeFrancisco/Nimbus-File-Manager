...
class SimilarityJobHandlerTest {
	...
	void eachHandlerRunsTheJobWithItsOwnAnalyzer() {
		...
		ExecutionOwnership ownership = Takings.owning(42L);

		photos.handle(execution, claimed, ownership);
		videos.handle(execution, claimed, ownership);

		verify(similarityJob).run(photoSimilarityService, execution, claimed, ownership);
		verify(similarityJob).run(videoSimilarityService, execution, claimed, ownership);
	}
}