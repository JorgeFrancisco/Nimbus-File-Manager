...
class OrganizationPreviewJobTest {
	...
	/**
	 * A preview needs no ownership of the folders it reads: nothing here writes to
	 * the library, and the executor's own gate blocks every side effect. Passing
	 * one would be claiming a right this job does not need.
	 */
	@Test
	void thePreviewRunsUnderNoOwnershipBecauseItWritesNothing() {
		whenTheExecutorPlans();
		when(organizationPlanWriter.publish(anyLong(), any())).thenReturn(true);

		job.run(request(), execution);

		verify(organizationExecutor).execute(any(), eq(execution), eq(null), any());
	}
	...
	void aPreviewThatDiesLeavesItsPlanUnpublishedAndMarkedFailed() {
		...
		Assertions.assertThatThrownBy(() -> job.run(request, execution)).isInstanceOf(IllegalStateException.class);
	...
	void losingThePublicationRaceDoesNotThrow() {
		...
		Assertions.assertThatCode(() -> job.run(request(), execution)).doesNotThrowAnyException();
	}
