...
class LeaseRenewerTest {
	...
	void renewsEveryHeldLeaseInOneCall() {
		...
		renewer.attemptStarted(1L, 1);
		renewer.attemptStarted(2L, 1);

		when(executionQueue.renewLeases(anyString(), anyList(), anyInt())).thenAnswer(answer -> answer.getArgument(1));

		renewer.renew();

		ArgumentCaptor<List<ExecutionPossession>> takings = ArgumentCaptor.captor();

		verify(executionQueue).renewLeases(anyString(), takings.capture(),
				eq(WorkerProperties.DEFAULT_LEASE_SECONDS));

		Assertions.assertThat(takings.getValue()).extracting(ExecutionPossession::executionId)
	...
	void stopsRenewingAnExecutionThatLostItsLockSession() {
		...
		verify(executionQueue).renewLeases(anyString(), takings.capture(), anyInt());

		Assertions.assertThat(takings.getValue()).extracting(ExecutionPossession::executionId).containsExactly(1L);
	}
	...
	void carriesOnWhenALeaseWasTakenAway() {
		...
		when(executionQueue.renewLeases(anyString(), anyList(), anyInt())).thenReturn(List.of());
	...
	private ExecutionOwnership ownership(long executionId, boolean stillOwned) {
		...
		when(ownership.mayGoOnWorking()).thenReturn(stillOwned);
