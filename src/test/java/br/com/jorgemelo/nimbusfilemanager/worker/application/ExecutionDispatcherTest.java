...
class ExecutionDispatcherTest {
	...
	/**
	 * What the dispatcher holds over a row it reserved but never began: the claim,
	 * and no taking of it. The writes made before an attempt is counted answer to
	 * this one.
	 */
	private ExecutionOwnership beforeAnyTaking;
	...
	void setUp() {
		...
		ownership = owning();
		beforeAnyTaking = owning();

		when(operationLockService.acquireFor(anyLong(), any(), any(Path[].class))).thenReturn(ownership);
		lenient().when(operationLockService.acquireNothingFor(anyLong())).thenReturn(beforeAnyTaking);
	...
	void handsTheExecutionBackWithoutSpendingAnAttemptWhenTheTreeIsBusy(@TempDir Path folder) {
		...
		verify(executionQueue).release(eq(7L), anyString(), any(), anyInt());
	...
	void chargesTheAttemptWhenSomethingOtherThanContentionFailsBeforeItRuns(@TempDir Path folder) {
		...
		verify(executionProgressService).fail(eq(beforeAnyTaking), any());
	}
	...
	void runsNothingWhenTheAttemptCannotBeCounted(@TempDir Path folder) {
		...
		when(executionQueue.countAttempt(eq(7L), anyString(), anyInt())).thenReturn(OptionalInt.empty());
	...
	/**
	 * The line between the two authorities this class writes under, asserted
	 * rather than trusted.
	 *
	 * <p>
	 * Before the attempt is counted there is no taking to name, so those writes go
	 * under the claim itself. From the moment {@code claim_count} moves there is
	 * one, and everything written from then on has to name it - a write that fell
	 * back to the takingless authority would be a write a later taking of the same
	 * row could not be told apart from, which is the whole of what the number is
	 * for. The handler here holds a path lock, so a takingless ownership has
	 * exactly one possible caller: the pre-attempt writes.
	 */
	@Test
	void asksForNoTakinglessOwnershipOnceTheAttemptHasBeenCounted(@TempDir Path folder) {
		reserved(folder);
		...
		verify(executionProgressService).fail(eq(ownership), any());
		verify(operationLockService, never()).acquireNothingFor(anyLong());
	}

	/**
	 * The window itself: the attempt is counted, the taking is born, and the very
	 * next thing that touches the database fails.
	 *
	 * <p>
	 * A passing failure is the case that matters, so the row refuses to be read
	 * once and answers the second time - which is what makes this the write that
	 * used to land under an authority carrying no number at all. Charging the
	 * attempt a second time would be the tell that it went the old way, so the
	 * count is asserted as well as the ownership.
	 */
	@Test
	void reportsUnderTheTakingWhenTheFailureLandsAfterTheAttemptWasCounted(@TempDir Path folder) {
		reserved(folder);

		when(executionQueue.countAttempt(eq(7L), anyString(), anyInt())).thenReturn(OptionalInt.of(1));
		when(executionRepository.findById(7L)).thenThrow(new IllegalStateException("the row could not be read"))
	...
	/**
	 * And the late write is a write like any other. Recovery gave the row back
	 * while this one was failing and the same worker took it again, so what the
	 * dispatcher hands to the progress service is a taking that has been replaced
	 * - and the service refuses it for that reason, not for having no number.
	 */
	@Test
	void theLateOutcomeCarriesATakingThatALaterOneHasAlreadyReplaced(@TempDir Path folder) {
		ExecutionOwnershipGuard guard = new ExecutionOwnershipGuard(mock(ExecutionQueue.class));

		ExecutionOwnership taking = Takings.owning(7L, guard);

		reserved(folder);

		when(operationLockService.acquireFor(anyLong(), any(), any(Path[].class))).thenReturn(taking);
		when(executionQueue.countAttempt(eq(7L), anyString(), anyInt())).thenReturn(OptionalInt.of(1));
		when(executionRepository.findById(7L)).thenAnswer(_ -> {
			guard.takes(new ExecutionPossession(7L, new WorkerIdentity().workerId(), 2));

			throw new IllegalStateException("the row could not be read");
		}).thenReturn(Optional.of(execution));

		dispatcher.dispatchOne();

		ArgumentCaptor<ExecutionOwnership> wrote = ArgumentCaptor.forClass(ExecutionOwnership.class);

		verify(executionProgressService).fail(wrote.capture(), any());
		verify(operationLockService, never()).acquireNothingFor(anyLong());

		assertThat(wrote.getValue()).as("the outcome is written under the taking the run was").isSameAs(taking);
		assertThat(guard.isTheCurrentTaking(7L, 1)).as("which the row no longer belongs to").isFalse();
		assertThat(guard.isTheCurrentTaking(7L, 2)).as("and the one that replaced it is untouched").isTrue();
	}

	/**
	 * The row went while the failure was being handled - recovery took it and a
	 * purge of finished executions removed it, or somebody deleted it. There is
	 * nothing left to write an outcome on, so the dispatcher says so and stops;
	 * inventing one on a row that is not there would be worse than the silence.
	 */
	@Test
	void saysSoAndStopsWhenTheRowIsGoneBeforeTheLateOutcomeCanBeWritten(@TempDir Path folder) {
		reserved(folder);
	...
	/**
	 * The invariant the boundary exists for, from its worst side: the outcome
	 * cannot be written either.
	 *
	 * <p>
	 * Nothing may leave, because what waits outside is the handler that writes
	 * under the claim alone - the authority this row stopped answering to the
	 * moment its attempt was counted. So the failure ends here, the lease lapses,
	 * and recovery closes the row, which is the frontier built for exactly that.
	 */
	@Test
	void lettingNothingEscapeWhenWritingTheLateOutcomeFailsToo(@TempDir Path folder) {
		reserved(folder);
		...
		doThrow(new IllegalStateException("and neither could it be written"))
				.when(executionProgressService).fail(any(), any());
	...
	/** The same line, for the hand-back a handler's own lock failure produces. */
	@Test
	void handsBackUnderTheTakingWhenTheHandlerFindsAFurtherPathBusy(@TempDir Path folder) {
		reserved(folder);

		when(executionQueue.countAttempt(eq(7L), anyString(), anyInt())).thenReturn(OptionalInt.of(1));
		when(executionQueue.release(eq(7L), anyString(), any(), anyInt())).thenReturn(false);
		when(executionQueue.hasWaitingDuplicate(7L)).thenReturn(true);

		doThrow(new OperationLockException("busy")).when(handler).handle(any(), any(), any());

		dispatcher.dispatchOne();

		verify(executionProgressService).reject(eq(ownership), any());
		verify(operationLockService, never()).acquireNothingFor(anyLong());
	}
	...
	void endsAPoisonJobRatherThanQueueingItInvisibly(@TempDir Path folder) {
		...
		verify(executionProgressService).fail(eq(ownership), any());
		verify(executionQueue, never()).release(anyLong(), anyString(), any(), anyInt());
	}
	...
	void refusesAnExecutionItsSuccessorHasAlreadyReplaced(@TempDir Path folder) {
		...
		verify(executionProgressService).reject(eq(beforeAnyTaking), any());
		verify(executionProgressService, never()).fail(any(), any());
	}
	...
	void endsAsInterruptedWhenTheHandlerFindsItsLocksGone(@TempDir Path folder) {
		...
		verify(executionProgressService).interrupt(eq(ownership), any());
		verify(executionProgressService, never()).fail(any(), any());
	}
	...
	void refusesToStartWorkWhenTheLocksWentAwayBeforeItBegan(@TempDir Path folder) {
		...
		when(executionQueue.countAttempt(eq(7L), anyString(), anyInt())).thenReturn(OptionalInt.of(1));
		doThrow(new OwnershipLostException("gone")).when(ownership).assertMayGoOnWorking();
	...
	private ExecutionOwnership owning() {
		...
		when(held.mayGoOnWorking()).thenReturn(true);
