...
## 9. Como App e Worker se encontram na instalação

A imagem instalada tem **dois launchers nativos** sobre um único jar e um único runtime:
`Nimbus File Manager.exe` é a App e `Nimbus File Manager Worker.exe` é o Worker, este último
sem atalho e sem entrada no menu — invisível como aplicativo, visível como processo. Cada um
tem seu `.cfg`, e é dele que o Worker tira o profile `worker`, o modo headless, a ausência de
migrations e o próprio heap.

O supervisor descobre esse launcher por **existência de arquivo**: a raiz da imagem é o pai de
`java.home`, e o Worker está lá ou não está. Fora de uma instalação — IDE, suíte — não está, e
o caminho de desenvolvimento continua sendo `java -cp`. Não se pergunta de onde a classe foi
carregada: dentro do jar empacotado essa pergunta devolve uma URI de arquivo aninhado, e a
tentativa de tratá-la como caminho derrubava a aplicação instalada antes de qualquer tela.

Duas coisas atravessam por argumento, e só duas. O **workspace efetivo** vai da App para o
Worker em todo start e todo restart, porque nem environment nem system property sobrevivem à
fronteira: o Windows dá ao processo elevado um environment novo, e uma JVM filha não herda
system properties. O **pid da App** vai junto, para o Worker não sobreviver a quem o
supervisiona. A conexão do banco **não** viaja: o Worker lê o `cluster.properties` do mesmo
workspace, que é a única fonte de verdade, e recusa-se a iniciar se ele não estiver lá — sem
cair no `localhost:5432` que costuma ser o PostgreSQL de outra coisa.

## 10. Limite conhecido: caracteres do caminho fora da codificação nativa

O workspace efetivo viaja entre os processos do Nimbus pela linha de comando: a App
o encaminha ao reinício elevado (jornal USN) e o passa a todo Worker que inicia ou
reinicia. Isso mantém uma fonte de verdade só, sem variável de ambiente que a
elevação apaga nem arquivo paralelo.

O limite está abaixo dessa camada. O launcher do Java no Windows decodifica a linha
de comando que recebeu usando a codificação nativa da máquina (`sun.jnu.encoding`,
Cp1252 numa instalação pt-BR), e o que não couber ali vira `?` **antes de `main`
existir**. Foi medido chamando `java.exe` diretamente, sem PowerShell, sem
`Start-Process` e sem elevação: o mesmo argumento se perde igual.

Na prática: acentuação do português, espaços, `&`, parênteses, apóstrofo, aspas e
barra invertida final atravessam intactos — isso é exercitado de ponta a ponta em
`WindowsCommandLineTest`. Um caminho com caracteres fora da codificação nativa, como
ideogramas numa máquina pt-BR, não atravessa.

Não é limitação do catálogo nem do filesystem do Nimbus: o produto lê e grava esses
arquivos normalmente. É limitação do transporte por linha de comando entre processos.
**O produto não pede que ninguém mude a code page do Windows** (a opção "Beta: Use
Unicode UTF-8") para contornar isso — seria alterar o locale da máquina inteira por
causa de um caminho. Se um JDK futuro deixar de perder esses caracteres, o teste que
hoje registra o limite passa a falhar, e a restrição sai com prova.